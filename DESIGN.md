...
```
https://companion.tvchat.ai/{sessionToken}?ctx=setup|chat
```
...
{ "type": "message_queued" }
{ "type": "model_state",   "model": "claude-sonnet-4-6", "models": [ { "id": "...", "label": "...", "badge": "..." } ] }
{ "type": "heartbeat" }
```

The TV pushes `model_state` when a phone peer joins and whenever the model changes (from the
TV settings panel or a phone `set_model`), so the phone's model picker stays in sync. The TV
is the single source of truth for the model list; the phone never hard-codes it. The TV
ignores any `set_model` whose `model` is not in its known list.
...
|-----|------|-------------|
| `tvchat_api_key` | `string` | Anthropic API key, plaintext |
| `tvchat_model` | `string` | Active model ID |
| `tvchat_conversations` | `string` (JSON) | Array of `Conversation` objects |
| `tvchat_active_id` | `string` | ID of currently open conversation |
...
```
VITE_RELAY_URL=wss://tvchat-relay.onrender.com
```

**Environment variable the TV app needs** (baked at build time):
```
REACT_APP_RELAY_URL=wss://tvchat-relay.onrender.com
REACT_APP_COMPANION_URL=https://companion.tvchat.ai
```
